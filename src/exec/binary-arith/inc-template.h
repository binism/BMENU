...
make_helper(concat(inc_r_,SUFFIX)) {
   ...
   REG(m.reg)=REG(m.reg) + 1;
   DATA_TYPE MSB_2 = MSB(REG(m.reg));
   inc_eflags_change(tmp,MSB_1,MSB_2);
   print_asm("inc" " $0x1,%%%s",REG_NAME(m.reg));
   return 1 + 1 ;
}
...
make_helper(concat(inc_rm_,SUFFIX)) {
	...
	if(m.mod == 3) {
        ...
        inc_eflags_change(tmp,MSB_1,MSB_2);
        print_asm("inc" " $0x1,%%%s",REG_NAME(m.R_M));
        return 1 + 1 ;
	}
 else {
        swaddr_t addr;
        ...
        inc_eflags_change(tmp,MSB_1,MSB_2);
        print_asm("inc" " $0x1,%%%s",ModR_M_asm);
        return len + 1;
     }	
}
