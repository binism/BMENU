...
make_helper(concat(sub_i2ra_,SUFFIX)) {
   ...
   cmp_eflags_change(imm,tmp, MSB_1, MSB_2, MSB_3);
   print_asm("sub" " $0x%x,%%%s",imm,REG_NAME(0x0));
   return DATA_BYTE + 1;
}
...
make_helper(concat(sub_r2rm_,SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
        if(m.mod == 3) {
           ...
           DATA_TYPE MSB_3 = MSB(REG(m.R_M));
           cmp_eflags_change(REG(m.reg),tmp,MSB_1,MSB_2,MSB_3);
...
make_helper(concat(sub_rm2r_,SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
        if(m.mod == 3) {
           ...
           cmp_eflags_change(REG(m.R_M),tmp,MSB_1,MSB_2,MSB_3);
           ...
           DATA_TYPE MSB_2 = MSB(REG(m.reg));
           REG(m.reg)=REG(m.reg)-MEM_R(addr);
           DATA_TYPE MSB_3= MSB(REG(m.reg));
           cmp_eflags_change(MEM_R(addr),tmp,MSB_1,MSB_2,MSB_3);
