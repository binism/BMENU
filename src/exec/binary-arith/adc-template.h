...
make_helper(concat(adc_i2ra_,SUFFIX)) {
   ...
   adc_eflags_change(imm,tmp,MSB_1,MSB_2,MSB_3);
   long long sum = tmp + imm + cpu.EFLAGS.CF;
   if(((sum>>(length+1)&0x1)==1))
	cpu.EFLAGS.CF=1;
   else
	cpu.EFLAGS.CF=0;
   print_asm("adc" " $0x%x,%%%s",imm,REG_NAME(0x0));
   return DATA_BYTE + 1;
} 
...
make_helper(concat(adc_i2rm_,SUFFIX)) {
	...
	imm =instr_fetch(eip + 1 + 1,DATA_BYTE);
        ...
        if(((sum>>(length+1)&0x1)==1))
		  ...
		  cpu.EFLAGS.CF=0;
        print_asm("adc" " $0x%x,%%%s",imm,REG_NAME(m.R_M));
        return 1 + 1 + DATA_BYTE;
          ...
          MEM_W(addr,REG(m.R_M) + imm + cpu.EFLAGS.CF);
          ...
          if(((sum>>(length+1)&0x1)==1))
	        ...
	        cpu.EFLAGS.CF=0;
          print_asm("adc" " $0x%x,%s",imm,ModR_M_asm);
...
make_helper(concat(adc_r2rm_,SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
        if(m.mod == 3) {
           ...
           DATA_TYPE MSB_3 = MSB(REG(m.R_M));
           adc_eflags_change(REG(m.reg),tmp,MSB_1,MSB_2,MSB_3);
           long long sum = tmp + REG(m.reg) + cpu.EFLAGS.CF;
           if(((sum>>(length+1)&0x1)==1))
	    cpu.EFLAGS.CF=1;
           else
	    cpu.EFLAGS.CF=0;
           print_asm("adc" " %%%s,%%%s",REG_NAME(m.reg),REG_NAME(m.R_M));
           return 2;       
	}
	else {
	   swaddr_t addr;
	   DATA_TYPE len = read_ModR_M(eip + 1, &addr);
           ...
           if(((sum>>(length+1)&0x1)==1))
	          ...
	          cpu.EFLAGS.CF=0;
          print_asm("adc" " %%%s, %s",REG_NAME(m.reg),ModR_M_asm);
          return len + 1;     
        }
}     
...
make_helper(concat(adc_rm2r_,SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
        if(m.mod == 3) {
           ...
           adc_eflags_change(REG(m.R_M),tmp,MSB_1,MSB_2,MSB_3);
           long long sum = tmp + REG(m.R_M) + cpu.EFLAGS.CF;
           if(((sum>>(length+1)&0x1)==1))
	    cpu.EFLAGS.CF=1;
           else
	    cpu.EFLAGS.CF=0;
           print_asm("adc" " %%%s,%%%s",REG_NAME(m.R_M),REG_NAME(m.reg));
           return 2;       
	}
	else {
	       swaddr_t addr;
           ...
           DATA_TYPE MSB_2 = MSB(REG(m.reg));
           REG(m.reg)=MEM_R(addr) + REG(m.reg) + cpu.EFLAGS.CF;//REG(m.reg) = MEM_R(addr);
           DATA_TYPE MSB_3 = MSB(REG(m.reg));
           adc_eflags_change(MEM_R(addr),tmp,MSB_1,MSB_2,MSB_3);
           long long sum = MEM_R(addr) + tmp + cpu.EFLAGS.CF;
           if(((sum>>(length+1)&0x1)==1))
	         cpu.EFLAGS.CF=1;
           else
	         cpu.EFLAGS.CF=0;
           print_asm("adc" " %s,%%%s",ModR_M_asm,REG_NAME(m.reg));
           return len + 1;     
        }
}     
