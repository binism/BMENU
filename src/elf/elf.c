...
int val_SYMBOL(char *SYMBOL) 
{
	int t;
	for (t=0;t<nr_symtab_entry;t++)
	{  if(symtab[t].st_info==0x11||symtab[t].st_info==0x12)
		{ if(strcmp(strtab+symtab[t].st_name,SYMBOL)==0)
			return symtab[t].st_value;
		}
	}
	return 0;
}
 bool find_func_name(swaddr_t func,char **func_name) {
	assert(func != 0);
	int i = 0;
